...
轨道 A（command transport review-only），见 ADR-0001 / ADR-0002 / v1.5b 实现交接。
web-dom 自动发送/提取已由 ADR-0002 标记为 superseded/deferred，除非后续 ADR 重新
批准，否则不作为活跃路线。

控制台 UI 的实现时机明确推迟到「轨道 A 机制 + 确认门在真实使用中被验证有价值」之后。

后续项目控制台、AgentTeam、多槽位执行、模型 API、harness、memory、WorkBuddy/qclaw/
openclaw/hermes 执行端点等扩展，必须同时参考
`PLAN-AGENTTEAM-PROJECT-CONTROL-PLANE.md` 与其原始讨论记录
`CLI-BRIDGE-v2.1-AGENTTEAM-DISCUSSION-RAW.md`，以及方向性评审
`CLI-BRIDGE-v2.1-AGENTTEAM-DIRECTIONAL-REVIEW.md`。这些文档已纳入 lint required
paths，用于防止后续开发遗漏该规划边界。
...
- 成员（当前/计划）：Codex CLI（第三方 API）、Claude Code（第三方 API）、OpenCode
  等受 capability gate 约束的执行端点。未来可扩展。
- 职责：真正动手——改文件、出 diff、推进任务状态。
- 能力约束：`canExecute` / `canAcceptPrompt` = true。

### 旁路 —— WorkBuddy（task source / result sink，非 executor）

- WorkBuddy **不是** executor，也不是 controller。它只作为任务来源与结果归档：
  提供 task / project 上下文，接收 review 结论与执行记录。
- WorkBuddy **不得**自行动手执行，也**不得**在回路内触发 Codex/Claude/OpenCode 执行
  （沿用 v0.8 WorkBuddy 硬边界）。
- 把它单列于 executor 之外，避免被误解为"WorkBuddy 可触发执行"。

注：以上描述是 WorkBuddy 当前已实现的 task-system 身份，不是永久产品上限。后续如
WorkBuddy、qclaw、openclaw、hermes 等工具要执行代码，必须作为**另一个单独注册的
execution endpoint identity** 进入 EndpointRegistry/AgentTeam，并接受 capability、
scope、isolation、gate、audit 约束；任务系统身份本身仍不得绕过中间层触发执行。详见
...
方向性评审后，tier × role / token economy 已降级为**可选路由优化**，不再是控制台的
核心产品叙事。近端核心应是项目级可观测性、上下文路由、受控执行、审计和可中断 gate。
...
若后续接入 AgentTeam 或多槽位执行，控制台应升级为 project control plane：展示 Goal、
PlanStep、AgentSlot、ExecutionObject、task board、conversation timeline、harness/
verification、audit、patch/merge queue，并在执行层不支持多槽位时明确提示不能并行。
...
- （web-dom 自动发送/提取为 deferred；若后续 ADR 重新批准并实现，再纳入启动条件。）
