...
**Status**: ADR-0017 and ADR-0018 ACCEPTED; ADR-0019 split into 0019-a CLOSED
and 0019-b ACCEPTED for `EX-2.14-2`
**Date**: 2026-06-13
**Produces**: ADR-0017 and ADR-0018 (ACCEPTED), ADR-0019-a (ACCEPTED & CLOSED),
ADR-0019-b (ACCEPTED for `EX-2.14-2`) + this execution roadmap
...
- **ADR-0019** must not be accepted or implemented until ADR-0018 is accepted
  ...
  offline, no network/credentials) is accepted and closed; **ADR-0019-b**
  (remote CI/GitHub + memory-only credentials) is accepted after
  `REVIEW-ADR-0019-b` PASS (2026-06-14) and remains bounded to `EX-2.14-2`.
  Provider scope/credential/redaction and URL containment blockers apply to
  0019-b only and are fixed in ADR-0019-b acceptance conditions.
...
| 7 | `EX-2.14-2` | execution | **ADR-0019-b** remote GitHub check-runs → typed pass/fail; operator identity + memory-only token; design FIXED (RP-2.14-b / RP-2.14-b-1), accepted by `REVIEW-ADR-0019-b`; dispatch only on explicit human trigger | `REVIEW-2.14-2` |
...
| 0019-b | Remote GitHub check status + memory-only credentials | high (network + creds) | ACCEPTED for `EX-2.14-2` | 2026-06-14 | REVIEW-ADR-0019-b |

**Group-acceptance semantics (historical hard rule).** "Accepting the bundle"
was bounded and did NOT promote every ADR to ACCEPTED:

- Only **ADR-0017** could be promoted to `ACCEPTED` on group acceptance, because it
  has no predecessor gate and no extra prerequisite review.
- **ADR-0018** and **ADR-0019** could at most be recorded as
  `ACCEPTED-INTENT — DEFERRED`. They were not set to `ACCEPTED` at group
  acceptance, because each still required, before its own acceptance:
...
- `ACCEPTED-INTENT` conferred no authorization to write code and did not satisfy
  any acceptance condition; it only recorded that the direction was endorsed
  pending the staged gates. Promotion of 0018/0019 to `ACCEPTED` was a separate,
  explicit senior-review decision recorded in this ledger at its own gate, after
  the unresolved sandbox/provider decisions named in §2 and §4 were fixed.
...
(`EX-2.14-1` / `REVIEW-2.14-1`, `c9afe51`). **ADR-0019-b** is ACCEPTED
(`REVIEW-ADR-0019-b` PASS, 2026-06-14) after RP-2.14-b and RP-2.14-b-1 fixed
the GitHub check-runs read-only provider design, operator identity, memory-only
operator-set token, HTTPS/TLS bounded egress, strict URL path containment, fixed
mapping, and redaction proof. Next: dispatch `EX-2.14-2` only on explicit human
trigger, returning to `REVIEW-2.14-2`. ADR-0019-b must never be merged into the
