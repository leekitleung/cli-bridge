 ...
 *
 * Verifies end-to-end: create loop, wait on unresolved cycle, continue after
 * result return, stop at max-cycles, extension session auth rejection.
 *
 * Usage:
...
async function main() {
  ...
  // 3. First tick dispatches one unresolved cycle
  ...
  // 4. A second tick before result return must wait, not dispatch
  const waitingTick = await handleBridgeRequest(runtime, 'POST',
    `/bridge/projects/cli-bridge/automation-loops/${loopId}/tick`,
    mockBody({ input: 'cycle two' }), undefined, CONSOLE_AUTH);
  assert.equal(waitingTick.statusCode, 200, 'waiting tick must succeed');
  const waiting = waitingTick.payload as { type: string };
  assert.equal(waiting.type, 'waiting', 'tick before result return must wait');

  const task1 = runtime.workbuddyExecution.claimNext('workbuddy');
  assert.ok(task1, 'first WorkBuddy task must be claimable');
  runtime.workbuddyExecution.recordResult(task1.taskId, {
    ok: true,
    proposalId: task1.proposalId,
    output: { nextInput: 'cycle two' },
    durationMs: 1,
  });

  // 5. After result return, next tick can dispatch cycle two using nextInput
  const tick2 = await handleBridgeRequest(runtime, 'POST',
    `/bridge/projects/cli-bridge/automation-loops/${loopId}/tick`,
    mockBody({}), undefined, CONSOLE_AUTH);
  ...
  const task2 = runtime.workbuddyExecution.claimNext('workbuddy');
  assert.ok(task2, 'second WorkBuddy task must be claimable');
  assert.equal(task2.prompt, 'cycle two', 'second task should use returned nextInput');
  runtime.workbuddyExecution.recordResult(task2.taskId, {
    ok: true,
    proposalId: task2.proposalId,
    output: {},
    durationMs: 1,
  });

  // 6. Third tick should stop with max-cycles
  ...
  // 7. Extension session cannot create/tick/run
  const extCreate = await handleBridgeRequest(runtime, 'POST',
    '/bridge/projects/cli-bridge/automation-loops',
    mockBody({ sourceEndpointId: 'chatgpt-web', targetEndpointId: 'workbuddy' }), undefined, EXT_AUTH);
  assert.equal(extCreate.statusCode, 403, 'extension session must 403 on create');
